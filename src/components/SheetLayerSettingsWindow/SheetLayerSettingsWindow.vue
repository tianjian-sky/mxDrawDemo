<template>
  <div class="sheet_layer_settings_window" v-show="isShow">
    <div class="sheet_layer_settings_window_header" @mousedown="onMousedown">
      <span class="sheet_layer-settings_window-header_title">图层</span>
      <div class="bf-close" @click="closeBox"></div>
    </div>
    <div class="sheet_layer_settings_window_header_table_head" >
      <div
        class="sheet_layer_settings_window_header_table_th"
    ...
    <div class="sheet_layer_settings_window_header_table_main sheet_layer_settings_window_container">
      <div
        class="sheet_layer_settings_window_header_table_tr"
        v-for="item in list"
        :key="item.id"
        
      >
        <div class="sheet_layer_settings_window_header_table_td flex_basis_50 iconfont" :class="item.off === 0 ? 'icon-xianshikejian': 'icon-yincangbukejian'" @click="onClickIsVisible(item)"></div>
        <div class="sheet_layer_settings_window_header_table_td flex_basis_50"> 
            <div class="color_box" :style="'background-color: ' + item.color"></div>
        </div>
        <div class="sheet_layer_settings_window_header_table_td flex_basis_auto">{{ item.name }}</div>
        <div class="sheet_layer_settings_window_header_table_td flex_basis_50 iconfont" :class="true ? 'icon-jiesuo': 'icon-suoding'" @click="onClickState(item)"></div>
      </div>
    </div>
  </div>
</template>
...
/**
 * @classdesc 图纸图层设置窗口
 * @abstract 用于控制和显示图层显示、颜色、名称、状态等信息
 *
 * */

@Component({
  name: "SheetLayerSettingsWindow",
})
...
/* 滚动容器 */
.sheet_layer_settings_window_container {
    overflow-y: auto;
    height: 300px;
}
/* 设置滚动条的样式 */
.sheet_layer_settings_window_container::-webkit-scrollbar{
    width: 8px;
}
.sheet_layer_settings_window_container::-webkit-scrollbar-thumb {
    border-radius: 10px;
    background: rgba(255, 255, 255, 0.3);
}
/*滚动槽*/
.sheet_layer_settings_window_container::-webkit-scrollbar-track{
...
.sheet_layer_settings_window_header_title {
  font-size: 14px;
  user-select: none;
...
.sheet_layer_settings_window_header_table_th {
   font-size: 14px;
   padding: 5px 0;
}
.flex_basis_50 {
...
.sheet_layer_settings_window_header_table_tr {
    width: 100%;
    display: flex;
...
.sheet_layer_settings_window_header_table_td {
    display: flex;
    align-items: center;
    height: 35px;
